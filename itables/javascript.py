...
def _datatables_repr_(df=None, tableId=None, **kwargs):
    ...
    try:
        ...
        with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'js', 'parse_functions.js')) as pf_file:
            parse_functions = pf_file.read()
...
""" + parse_functions + """
        var dt_args = """ + dt_args + """;
        dt_args = parse_functions(dt_args);
